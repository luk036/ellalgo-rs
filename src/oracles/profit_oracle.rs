...
impl OracleOptim<Arr> for ProfitOracle {
    ...
    /// * `tea`: The parameter `tea` is a mutable reference to a `f64` variable.
...
impl OracleOptim<Arr> for ProfitOracleRB {
    ...
    /// `tea` based on the elasticities and returns a cut and the updated best-so-far value.
    ///
    /// Arguments:
    ///
    /// * `y`: The parameter `y` is an input quantity represented as an array (`Arr`) in log scale.
    /// * `tea`: The parameter `tea` is the best-so-far optimal value. It is passed as a mutable reference
...
impl OracleOptimQ<Arr> for ProfitOracleQ {
    ...
    /// optimal value `tea`, and returns a cut and the updated best-so-far value.
    ///
    /// Arguments:
    ///
    /// * `y`: The parameter `y` is an input quantity in log scale. It is of type `Arr`, which is likely an
    /// array or vector type.
    /// * `tea`: The parameter `tea` represents the best-so-far optimal value. It is a mutable reference to
