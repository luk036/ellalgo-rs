...
impl OracleOptim for ProfitOracle {
    fn assess_optim(&self, y: ArrayView1<f64>, tea: f64) -> (Cut, Option<f64>) {
        let fj: f64;
        let mut g: Arr;
        ...
        if (fj = f64::ln(tea + vx) - log_Cobb) >= 0.0 {
            g = q / (tea + vx) - &self.elasticities;
            return ((g, fj), None);
        }
        let tea = f64::exp(log_Cobb) - vx;
        g = q / (tea + vx) - &self.elasticities;
        ((g, 0.0), Some(tea))
    }
}
...
fn main() {
    ...
    let y = arr1(&[1.0, 2.0]);
    let tea = 3.0;
    let (cut, opt) = oracle.assess_optim(y.view(), tea);
}
